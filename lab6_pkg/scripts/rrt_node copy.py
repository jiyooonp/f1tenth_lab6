...
class RRT(Node):
    ...
    def scan_callback(self, msg):
        ...
        valid_indices_x = grid_x != self.grid_size - 1
        valid_indices_y = grid_y != self.grid_size - 1
    ...
    def rrt(self):
        ...
        if self.is_goal(new_node):
            ...
            for i in range(len(path) - 1):
                ...
                # add four points between each pair of points
                for j in range(self.interpolation_factor):
                    interpolated_path.append([path[i].x + (path[i+1].x - path[i].x) * j / self.interpolation_factor, path[i].y + (path[i+1].y - path[i].y) * j / self.interpolation_factor])
