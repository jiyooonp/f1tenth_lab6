...
class RRT(Node):
    ...
    def __init__(self):
        ...
        self.tf_listener = TransformListener(self.tf_buffer, self)
                self.transformation_map_to_base = self.transform_goal_point()
        self.transformation_base_to_map = self.transform_goal_point(src_frame='ego_racecar/base_link', dst_frame='map')
        ...
        if self.is_goal(new_node):
            ...
            for i in range(len(path) - 1):
                ...
                goal_point_base_link = do_transform_point(
                    PointStamped(point=path[i]), self.transformation_base_to_map)
                interpolated_path.append(goal_point_base_link.point)

                goal_point_base_link = do_transform_point(
                    PointStamped(point=Point(x=(path[i].x + path[i+1].x)/2, y=(path[i].y + path[i+1].y)/2, z=0.0)), self.transformation_base_to_map)
                interpolated_path.append(goal_point_base_link.point)
                
            goal_point_base_link = do_transform_point(
                PointStamped(point=path[-1]), self.transformation_base_to_map)
            interpolated_path.append(goal_point_base_link.point)
    ...
    def steer(self):
        ...
        goal_point_base_link = do_transform_point(
            PointStamped(point=self.goal_point_map), self.transformation_map_to_base)
    ...
    def transform_goal_point(self, src_frame='map', dst_frame='ego_racecar/base_link'):
        ...
        try:
            ...
            self.get_logger().info(
                f'Could not transform {src_frame} to {dst_frame}: {ex}')
            return
